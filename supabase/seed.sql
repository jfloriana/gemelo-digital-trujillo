...
-- 1) ZONAS (6)
insert into public.urban_zones (id, name, district, description, vulnerability_level, target_population, vulnerable_population, baseline_temp, baseline_pm25, tree_cover, built_density, primary_pollution_source, geometry_coords, sensors_count)
values
  ('zona-centro','Zona 1: Centro Histórico (Av. España / Jr. Pizarro)','Trujillo Cercado','Cañón urbano estrecho colonial con alta densidad peatonal y comercial, tráfico congestionado en anillo perimétrico Av. España y baja presencia de arbolado nativo.','Muy Alta',42500,14200,29.4,48.2,5.8,84.5,'Tráfico vehicular pesado/combis, cañón urbano con recirculación de vórtice y baja dispersión.','[{"x":10,"y":10},{"x":90,"y":10},{"x":90,"y":90},{"x":10,"y":90}]',4),
  ('zona-mayorista','Zona 2: Corredor Av. América Sur / Mercado Mayorista','Trujillo Cercado / El Bosque','Punto crítico comercial y logístico de alto tránsito diésel pesado, microbuses, mototaxis y carga continua con alta emisión de material particulado y calor antropogénico.','Crítica',68000,22800,31.8,64.7,3.2,91.0,'Flota de carga pesada diésel, paraderos informales y asfalto degradado con alta radiación térmica.','[{"x":15,"y":20},{"x":85,"y":15},{"x":80,"y":85},{"x":20,"y":80}]',5),
  ('zona-mansiche','Zona 3: Intercambio Vial Av. Mansiche / Mall Plaza','Trujillo / Huanchaco','Nodo vial metropolitano con alta velocidad y volumen vehicular continuo, grandes explanadas de pavimento asfaltado y estacionamientos que generan isla de calor.','Media',35000,9500,28.6,38.5,11.4,72.0,'Flujo vehicular continuo alta velocidad, calor reflejado por cubiertas metálicas y pavimentos.','[{"x":5,"y":15},{"x":95,"y":10},{"x":90,"y":85},{"x":10,"y":90}]',3),
  ('zona-porvenir','Zona 4: Distrito El Porvenir (Sector Industrial Calzado)','El Porvenir','Zona de microempresas de calzado y curtiembres con calderas artesanales, calles con déficit de pavimentación y cobertura vegetal casi nula (<2.5%).','Crítica',115000,41000,32.5,72.3,2.1,94.0,'Combustión industrial, solventes COVs, polvo resuspendido y alta radiación superficial.','[{"x":20,"y":10},{"x":95,"y":25},{"x":85,"y":95},{"x":10,"y":85}]',4),
  ('zona-victor-larco','Zona 5: Víctor Larco Herrera (Av. Larco / Huamán)','Víctor Larco','Corredor residencial-comercial con influencia de brisa marina vespertina pero con episodios matutinos de inversión térmica y congestión en horas punta.','Media',52000,16000,26.8,32.1,14.6,65.0,'Tráfico de transporte público y particular hacia balneario de Huanchaco/Buenos Aires.','[{"x":10,"y":15},{"x":90,"y":20},{"x":85,"y":85},{"x":15,"y":80}]',3),
  ('zona-hermelinda','Zona 6: Complejo La Hermelinda (Av. Villarreal / 8 de Octubre)','Florencia de Mora / Trujillo','Mercado de abastos informal de mayor escala en el norte del país, acumulación de residuos, tráfico desordenado y alto índice de partículas gruesas y finas.','Muy Alta',85000,29000,31.2,68.9,1.8,92.5,'Tráfico logístico incesante, residuos orgánicos e inorgánicos, quema clandestina periférica.','[{"x":15,"y":10},{"x":88,"y":18},{"x":82,"y":92},{"x":12,"y":88}]',4)
on conflict (id) do update set
  name = excluded.name, district = excluded.district, description = excluded.description,
