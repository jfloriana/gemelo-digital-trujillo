        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="md:col-span-2">
              ...
              <input value={form.name} onChange={e= autoComplete="off">setForm({...form, name:e.target.value})} placeholder="Zona 7: Av. Fátima / UPAO" className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
            </div>
            <div>
              <label className="text-xs font-semibold">Distrito *</label>
              <input value={form.district} onChange={e= autoComplete="off">setForm({...form, district:e.target.value})} placeholder="Trujillo" className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" />
            </div>
            <div>
            ...
            <div><label className="text-xs font-semibold">Población total</label><input type="number" value={form.targetPopulation} onChange={e= autoComplete="off">setForm({...form, targetPopulation: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">Población vulnerable</label><input type="number" value={form.vulnerablePopulation} onChange={e= autoComplete="off">setForm({...form, vulnerablePopulation: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">Temp base (°C)</label><input type="number" step="0.1" value={form.baselineTemp} onChange={e= autoComplete="off">setForm({...form, baselineTemp: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">PM2.5 base (µg/m³)</label><input type="number" step="0.1" value={form.baselinePM25} onChange={e= autoComplete="off">setForm({...form, baselinePM25: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">Cobertura arbórea (%)</label><input type="number" step="0.1" value={form.treeCover} onChange={e= autoComplete="off">setForm({...form, treeCover: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">Densidad edificada (%)</label><input type="number" step="0.1" value={form.builtDensity} onChange={e= autoComplete="off">setForm({...form, builtDensity: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div className="md:col-span-2"><label className="text-xs font-semibold">Fuente principal de contaminación</label><input value={form.primaryPollutionSource} onChange={e= autoComplete="off">setForm({...form, primaryPollutionSource:e.target.value})} placeholder="Tráfico, industria, polvo..." className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
          </div>
