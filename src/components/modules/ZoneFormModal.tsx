...
export const ZoneFormModal: React.FC<Props> = ({ isOpen, onClose, editingZone, onSaved }) => {
  ...
  const handleSave = async () => {
    setError(null);
    if (!form.name?.trim() || !form.district?.trim() || !form.description?.trim()) { setError(t('zoneForm.requiredError')); return; }
    setSaving(true);
    ...
    <div className="fixed inset-0 z-[80] bg-black/50 backdrop-blur-sm grid place-items-center p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-auto shadow-2xl">
        <div className="sticky top-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-5 py-4 flex items-center justify-between">
          <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2"><MapPin className="w-4 h-4 text-emerald-600" />{editingZone ? t('zoneForm.titleEdit') : t('zoneForm.titleNew')}</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="md:col-span-2">
              <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('zoneForm.name')}</label>
              <input value={form.name} onChange={e=>setForm({...form, name:e.target.value})} placeholder={t('zoneForm.placeholderName')} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
            </div>
            <div>
              <label className="text-xs font-semibold">{t('zoneForm.district')}</label>
              <input value={form.district} onChange={e=>setForm({...form, district:e.target.value})} placeholder={t('zoneForm.placeholderDistrict')} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" />
            </div>
            <div>
              <label className="text-xs font-semibold">{t('zoneForm.department')}</label>
            ...
            <div>
              <label className="text-xs font-semibold">{t('zoneForm.vulnerability')}</label>
              <select value={form.vulnerabilityLevel} onChange={e=>setForm({...form, vulnerabilityLevel: e.target.value as any})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs">
                <option>Media</option><option>Alta</option><option>Muy Alta</option><option>{t('zoneForm.vulnerability')==='Vulnerabilidad' ? 'Crítica' : 'Critical'}</option>
              </select>
            </div>
            <div className="md:col-span-2">
            ...
            <div><label className="text-xs font-semibold">{t('zoneForm.tempBase')}</label><input type="number" step="0.1" value={form.baselineTemp} onChange={e=>setForm({...form, baselineTemp: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">{t('zoneForm.pmBase')}</label><input type="number" step="0.1" value={form.baselinePM25} onChange={e=>setForm({...form, baselinePM25: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">{t('zoneForm.treeCover')}</label><input type="number" step="0.1" value={form.treeCover} onChange={e=>setForm({...form, treeCover: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div><label className="text-xs font-semibold">{t('zoneForm.builtDensity')}</label><input type="number" step="0.1" value={form.builtDensity} onChange={e=>setForm({...form, builtDensity: Number(e.target.value)})} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
            <div className="md:col-span-2"><label className="text-xs font-semibold">{t('zoneForm.source')}</label><input value={form.primaryPollutionSource} onChange={e=>setForm({...form, primaryPollutionSource:e.target.value})} placeholder={t('zoneForm.placeholderSource')} className="w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={onClose} className="px-4 py-2 text-xs font-semibold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50">{t('zoneForm.cancel')}</button>
            <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl flex items-center gap-1.5 disabled:opacity-50"><Save className="w-3.5 h-3.5" />{saving ? t('zoneForm.saving') : editingZone ? t('zoneForm.update') : t('zoneForm.save')}</button>
          </div>
        </div>
      </div>
    </div>
