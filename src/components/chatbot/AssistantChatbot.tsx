...
export const AssistantChatbot: React.FC<AssistantChatbotProps> = ({
  ...
  const toggleListening = () => {
    ...
    if (!SpeechRecognitionClass) {
      alert('El reconocimiento de voz no está soportado en este navegador. Te recomendamos usar Google Chrome, Edge o Safari.');
      return;
    }
    ...
    try {
      ...
      recognition.onstart = () => {
        setIsListening(true);
        setVoiceStatusText('🎙️ Escuchando... habla ahora en español');
      };
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event.error === 'not-allowed') {
          setVoiceStatusText('⚠️ Permiso de micrófono denegado. Permite el acceso en el navegador.');
        } else {
          setVoiceStatusText(null);
        }
      };
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'msg-welcome',
      sender: 'bot',
      text: `¡Hola ${user?.name ? user.name.split(' ')[0] : 'colega'}! 👋 Soy **EcoTwin Bot**, el Asistente Virtual Inteligente del **Gemelo Digital de Trujillo 2026**.\n\nPuedo guiarte paso a paso por la investigación de tesis, resolver dudas científicas o ayudarte a simular intervenciones de Soluciones Basadas en la Naturaleza (NbS).`,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      quickActions: [
        { label: '🚀 Tour Guiado del Gemelo', actionType: 'trigger_guided', target: 'tour' },
        { label: '🌳 Asistente de Simulación NbS', actionType: 'trigger_guided', target: 'nbs_guide' },
        { label: '📡 Calibración de Sensores IoT', actionType: 'trigger_guided', target: 'iot_guide' },
        { label: '🤖 Comparar Modelos IA (1D-CNN)', actionType: 'navigate', target: 'ai_engine' },
        { label: '📄 Generar Informe PDF/Excel', actionType: 'navigate', target: 'reports' }
      ]
    }
  ]);
      ...
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-[60] flex items-center gap-3">
          ...
          <button
            ...
            title="Abrir Asistente Virtual Inteligente (EcoTwin Bot)"
          >
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-sm text-white">EcoTwin Bot Trujillo</h3>
                ...
                <p className="text-[11px] text-slate-300 flex items-center gap-1.5 mt-0.5">
                  ...
                  Asistente Científico & Guía Paso a Paso
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 text-slate-300">
              ...
              {speakingMessageId && (
                <button
                  ...
                  title="Detener voz del asistente"
                >
                  ...
                  {msg.sender === 'bot' && (
                    <div className="flex items-center justify-between pb-1.5 mb-1.5 border-b border-slate-100/80">
                      ...
                      <button
                        ...
                        title={speakingMessageId === msg.id ? 'Detener lectura' : 'Escuchar respuesta por voz'}
                      >
                        {speakingMessageId === msg.id ? (
                          <>
            ...
            {isTyping && (
              <div className="flex gap-3 items-center">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl px-4 py-2.5 text-xs text-slate-500 flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-slate-400 ml-1">Procesando consulta...</span>
                </div>
              </div>
            )}
          ...
          >
            ...
            <button
              ...
              title={isListening ? 'Detener dictado por voz' : 'Dictar por voz en español (Micrófono)'}
            >
              {isListening ? (
  ...
  // Gemini 2.0 Flash integration helpers (added for varied responses)
  const getGeminiKey = () => import.meta.env.VITE_GEMINI_API_KEY || (import.meta as any).env.GEMINI_API_KEY || {};
  const buildSystemPrompt = (selectedZone, activeScenario, tone) => { const zone = selectedZone ? `${selectedZone.name}: PM2.5 ${selectedZone.baselinePM25}µg, Temp ${selectedZone.baselineTemp}°C` : "Trujillo (sin zona seleccionada)"; const scenario = activeScenario ? `- Escenario: ${activeScenario.code}` : ""; const toneIns = tone === "cientifico" ? "tono científico académico" : tone === "municipal" ? "tono municipal/ordenanza" : "tono didáctico"; return `Eres EcoTwin Bot. Contexto: ${zone} ${scenario}. Responde con ${toneIns}. Siempre incluye datos concretos (números, porcentajes, unidades).`; };
  const generateWithGemini = async (query, history) => { const key = getGeminiKey(); if (!key) return null; try { const ai = new GoogleGenerativeAI({ apiKey: key }); const sp = buildSystemPrompt(selectedZone, activeScenario, assistantTone); const model = ai.getGenerativeModel({ model: "gemini-2.0-flash", systemInstruction: sp }); const parts = []; const recent = history.slice(-6); for (const m of recent) { const role = m.sender === "user" ? "user" : "model"; parts.push({ role, parts: [{ text: m.text }] }); } parts.push({ role: "user", parts: [{ text: query }] }); const res = await model.generateContent({ contents: parts, generationConfig: { temperature: 0.75, topP: 0.9, maxOutputTokens: 512 } }); return { id: `bot-${Date.now()}`, sender: "bot", text: res.text || "Respuesta vacía", timestamp: new Date().toLocaleTimeString(), quickActions: [] }; } catch (e) { console.warn("Gemini fallback", e); return null; } };
  const detectIntent = (q) => { const lower = q.toLowerCase(); if (lower.includes("diagnostico") || lower.includes("oe1") || lower.includes("zonas") || lower.includes("mapa")) return "diagnostico"; if (lower.includes("sensor") || lower.includes("iot") || lower.includes("calibracion") || lower.includes("oe2")) return "sensor"; if (lower.includes("ia") || lower.includes("ai") || lower.includes("modelo") || lower.includes("1d-cnn") || lower.includes("gnn") || lower.includes("oe3")) return "ia"; if (lower.includes("simular") || lower.includes("simulador") || lower.includes("nbs") || lower.includes("oe4")) return "nbs"; if (lower.includes("reporte") || lower.includes("exportar")) return "reporte"; if (lower.includes("tesis") || lower.includes("oe5")) return "tesis"; if (lower.includes("hola") || lower.includes("saludos")) return "saludo"; return "general"; };
  const VARIANTS = { saludo: ["¡Hola ${name}! 👋 Soy EcoTwin Bot.", "¡Saludos ${name}! 🤖 Estoy aquí para ayudarte."].map(v => v.replace("${name}", user?.name ? user.name.split(" ")[0] : "colega")), diagnostico: ["📍 Módulo OE1: Diagnóstico de Microescala - explores la línea base ambiental de las 6 zonas de Trujillo.", "🌡️ Diagnóstico microescala - Identificamos islas de calor y puntos críticos de PM2.5 en 6 zonas clave."], sensor: ["📡 Módulo OE2: Arquitectura IoT y Calibración - supervisamos telemetría en tiempo real y algoritmo de calibración que eleva R² a 0.94.", "🧪 Calibración 2-etapas - Método Zhivkov corrige sobrestimación por humedad, llevando R² de 0.29 a 0.94."], ia: ["🤖 Módulo OE3: Modelos de IA & Benchmarks - comparamos 5 arquitecturas: 1D-CNN R²=0.9925 (4.8ms), GNN R²=0.9510, RF R²=0.8870.", "🧠 Comparación de modelos IA - El 1D-CNN de Naveed 2025 lidera con R²=0.9925 y 4.8ms de inferencia."], nbs: ["🌳 Módulo OE4: Simulador NbS - modifies cañón urbano (H/W), viento, radiación, arbolado nativo (Molle/Huarango), techos verdes GREENPASS®.", "💚 Intervenciones NbS - Arbolado nativo reduce PM2.5 28.5% y baja temperatura 3.2°C."], reporte: ["📄 Generador de Reportes - exporto al instante a Excel (.xlsx), PDF, Word (.docx) o CSV los datos del Gemelo Digital.", "📥 Reporte automático - synthesis con PM2.5, temperatura, reducción proyectada por NbS y costo en PEN."], tesis: ["🎓 Información de la Tesis - 5 OBJETIVOS ESPECÍFICOS (OE1-OE5) completados 100%. Hipótesis general validada p<0.001. 132,500 hab beneficiadas. B/C Ratio 3.4.", "✅ Validación de hipótesis - La tesis demuestra que el gemelo digital con NbS reduce exposición a contaminantes y calor extremo en Trujillo significativamente."], general: ["Entendido. Con respecto a ${query}, en el marco del Gemelo Digital de Trujillo podemos abordarlo desde varios frentes científicos:", "Sobre ${query} podemos evaluar: microescala y cañón urbano con relación H/W y vórtices de viento, inferencia predictiva con algoritmo 1D-CNN (R²=0.9925), intervenciones NbS con arbolado y techos verdes, o generación documental a Excel/PDF."].map(v => v.replace("${query}", query)) };
  const getVariedFallback = (intent) => { const pool = VARIANTS[intent] || VARIANTS.general; const selected = pool[Math.floor(Math.random * pool.length)]; return { id: `bot-${Date.now()}`, sender: "bot", text: selected, timestamp: new Date().toLocaleTimeString(), quickActions: [] }; };
  const quickActionsFromIntent = (intent) => { const base = []; switch (intent) { case "diagnostico": base.push({ label: "👉 Ir a Diagnóstico Microescala", actionType: "navigate", target: "diagnosis" }, { label: "📊 Ver Zonas en Excel", actionType: "export", target: "xlsx" }); break; case "sensor": base.push({ label: "👉 Ir a Arquitectura IoT (OE2)", actionType: "navigate", target: "architecture" }, { label: "🧪 Explicar Algoritmo de Calibración", actionType: "trigger_guided", target: "iot_guide" }); break; case "ia": base.push({ label: "👉 Ir al Motor de IA (OE3)", actionType: "navigate", target: "ai_engine" }, { label: "📈 Ver Tabla Comparativa de Modelos", actionType: "navigate", target: "ai_engine" }); break; case "nbs": base.push({ label: "👉 Ir al Simulador NbS (OE4)", actionType: "navigate", target: "nbs_simulator" }, { label: "🎯 Asistente Guiado de Planes NbS", actionType: "trigger_guided", target: "nbs_guide" }); break; case "reporte": base.push({ label: "📗 Descargar Libro Excel (.xlsx)", actionType: "export", target: "xlsx" }, { label: "📕 Descargar Informe Técnico (.pdf)", actionType: "export", target: "pdf" }); break; case "saludo": base.push({ label: "🚀 Iniciar Tour Guiado", actionType: "trigger_guided", target: "tour" }, { label: "🌳 Asistente de Simulación NbS", actionType: "trigger_guided", target: "nbs_guide" }, { label: "📊 Descargar Informe Completo", actionType: "export", target: "pdf" }); break; default: base.push({ label: "🌳 Simular en Zona Activa", actionType: "navigate", target: "nbs_simulator" }, { label: "🧠 Consultar Modelos de IA", actionType: "navigate", target: "ai_engine" }, { label: "📡 Ver Calibración IoT", actionType: "navigate", target: "architecture" }, { label: "📄 Exportar Reporte Técnico", actionType: "export", target: "pdf" }); }; return base; };
